...
def get_jacobian_matrix(model):
    """Get the Jacobian matrix for the model as Sympy expressions.

    Parameters
    ----------
    model : pysb.Model
        Model to get the Jacobian matrix for. If model.odes is empty (ODEs
        have not be generated yet), the ODEs will first be generated by calling
...
class Solver(object):
    ...
    def __init__(self, model, tspan, use_analytic_jacobian=False,
        ...
        if self._use_analytic_jacobian:
            ...
            if not Solver._use_inline:
                jac_eqs_py = compile(jac_eqs, '<%s jacobian>' %
                                     self.model.name, 'exec')
            else:
